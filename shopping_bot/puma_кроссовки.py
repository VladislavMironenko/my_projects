...
async def puma_sneakers(size , gender):
    url = f'https://ua.puma.com/ru/{gender}/obuv/krossovki.html?size={size}'
    option = Options()
    ...
    try:
        ...
        while True:
            try:
                ...
                # Если элемент найден, тогда выполняем сбор данных
                res = driver.find_elements(By.CLASS_NAME, 'grid__item.image-sv01')
                # time.sleep(random.randint(1, 3))
                for r in res:
                    r_price_sale = r.find_element(By.CLASS_NAME, 'price-container.price-final_price.tax.weee')
                    if r_price_sale is not None:
                        r_title = r.find_element(By.CLASS_NAME, 'product-item__name').text.strip()
                        r_price = r.find_element(By.CLASS_NAME, 'old-price.sly-old-price.no-display').text.strip()
                        r_price_sale = r_price_sale.text.strip()
                ...
                with open('result_puma_sneakers.json', 'w', encoding='utf-8') as f:
