...
async def puma_tshirts(size , gender):
    url = f'https://ua.puma.com/ru/{gender}/odezhda/futbolki-i-majki.html?size={size}'
    option = Options()
    ...
    try:
        ...
        while True:
            try:
                ...
                for r in res:
                    r_price = r.find_element(By.CLASS_NAME, 'old-price.sly-old-price.no-display')
                    if r_price is not None:
                        r_title = r.find_element(By.CLASS_NAME, 'product-item__name').text.strip()
                        r_price = r_price.text.strip()
                        r_price_sale = r.find_element(By.CLASS_NAME, 'price-container.price-final_price.tax.weee').text.strip()
                ...
                with open('result_puma_tshirts.json', 'w', encoding='utf-8') as f:
