...
async def adidas_tshirt(size , gender):
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36'
    }
    url = f'https://www.adidas.ua/{gender}/odyag/futbolki/{size}?page=1'
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(1, q+1):
            URL = f'https://www.adidas.ua/{gender}/odyag/futbolki/{size}?page={i}'
    ...
    with open('result_adidas_tshirt.json', 'w' , encoding='utf-8') as f:
        json.dump(lst , f , indent=4 , ensure_ascii=False)
    print('Сбор информации завершен')


def main():
    asyncio.run(adidas_tshirt())
